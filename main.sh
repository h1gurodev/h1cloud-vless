    ...
    main_inbound["streamSettings"] = {
        ...
        "xhttpSettings": {
            ...
            "extra": {
                ...
                "scMinPostsIntervalMs": 70,
                "uplinkDataPlacement": "body",
                "xmux": {"cMaxLifetimeMs": 0, "cMaxReuseTimes": "36-96", "maxConcurrency": "8-32", "maxConnections": 0, "hMaxRequestTimes": "320-640", "hMaxReusableSecs": "720-1800"},
            }
        }
    }
else:
...
def render_client_page(user, links, request_host=""):
    ...
    # Customization (set via `vpn sub title/desc/support/accent`), passed by env.
    page_title = (os.environ.get("SUB_PAGE_TITLE", "") or "").strip() or SUB_NAME or "H1Cloud Client"
    page_desc = (os.environ.get("SUB_PAGE_DESC", "") or "").strip()
    support_url = (os.environ.get("SUB_SUPPORT_URL", "") or "").strip()
    support_text = (os.environ.get("SUB_SUPPORT_TEXT", "") or "").strip() or "Поддержка"
    accent = (os.environ.get("SUB_ACCENT", "") or "").strip()
    # Accent: accept "#hex" or "#hex,#hex" (gradient). Fall back to default green/cyan.
    if accent:
        stops = [s.strip() for s in accent.split(",") if s.strip()]
        if len(stops) == 1:
            stops.append(stops[0])
        accent_css = f"linear-gradient(135deg,{stops[0]},{stops[1]})"
    else:
        accent_css = "linear-gradient(135deg,#4ade80,#22d3ee)"
    desc_html = f"<p>{html.escape(page_desc)}</p>" if page_desc else ""
    support_html = (
        f'<a href="{html.escape(support_url, quote=True)}" target="_blank" rel="noopener">{html.escape(support_text)}</a>'
        if support_url else ""
    )
...
<head>
  ...
  <style>
    ...
    a, button {{ color: #061018; background: {accent_css}; border: 0; border-radius: 9px; padding: 10px 12px; font-weight: 700; text-decoration: none; cursor: pointer; }}
...
<main>
  <section>
    ...
    {desc_html}
    <p>{html.escape(str(user.get("name", "")))} — подписка. Ссылка без token: <code>{html.escape(sub_url)}</code></p>
    <div class="actions">
